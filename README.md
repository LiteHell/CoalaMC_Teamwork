...
* select_block_constants
    - LHM_constants.h안에 있는 변수들을 불러와 블럭을 생성할 때 <code>locate</code>뒤에 오는 단어가 달라 이것을 지정하기 위해서 따로 빼둔 함수이다. 함수로 따로 빼지 않으면 이 구문이 필요한 부분마다 10줄에 가까운 코드를 매번 써줘야한다.
...
* 배 위에 수영장, 건물등을 설치한다.
* x,y,z값을 입력한다.
* 크기는 x축방향으로 x에서 x+27까지 지어진다(28) z축 방향으로 (z에서z+14)까지 (15)지어진다.


* createBuilding_HAPPY
* 2층의 구조가 유리로 돼있는 형태를 만들고, 잔디를 설치한다.
* 옥상에 수영장이 겸비된 금색 집을 만든다
* x,y,z값을 입력한다.
*크기는 x축 방향으로 x에서x+11까지 지어진다 (12) z축 방향으로 zz+27(28)개의 크기이다.
...
* createBuilding_JGH(int sx, int sy, int sz)
	...
	* 높이 4의 유리 건물 위에, 정육면체, 구, 피라미드의 3가지 도형을 무작위로, 무작위의 색깔로 8개 생성한다.
	
* createBuilding_JGH_SECOND(int sx, int sy, int sz)
	* 장기훈의 두번째 건물을 제작한다.
	* sx, sy, sz는 건물 생성의 시작점의 좌표이다.
	* 시작점의 좌표로부터 x축의 +방향으로, z축의 +방향으로 건물을 생성한다.
	* 건물의 x축 방향의 크기는 21, z축 방향의 크기도 21이다.
	* sy는 4로 하는 것이 여러모로 이롭다.
	* 중요 : 입구가 있는 면(문 달린 면)은 x좌표가 sx인 면과, 그 면과 평행한 면으로 총 2면이다.
	* 유리 텐트에 3가지 무작위 종류의 나무가 틀을 이룬 건물을 생성한다.
...
# create_random_bool함수
* 이 함수는 호출되면 0혹은 1을 return하는 함수이다.
* 함수의 목적은 랜덤으로 참 거짓을 판별하기 위함이다.
* randutils.cpp안에 있다.
* 제작자 - 신연진
...
* select_tree함수
    - x, y, z, n을 입력받는 함수이다.
    - 이 함수는 1 ~ 4번까지있는 park_tree_n함수를 선택해주는 함수이다. n의 값을 랜덤으로 입력 받는다.
    
* locate_park_tree함수
    - 이 함수는 공원의 중심으로 기준으로 나무를 랜덤으로 생성해주는 함수입니다.
    - 기본 원리는 park_lake함수를 기반으로 합니다.

* locate_tree_around함수
    - 이 함수는 공원을 둘러싼 나무를 만드는 함수입니다.
    - 공원을 생성할 장소를 초기화할 때 나무도 함께 없어지므로 이함수를 이용해 다시 나무를 만들어 줍니다.
    
* park_tree함수
    - 공원 각 꼭짓점을 기준으로 나무를 생성해주는 함수입니다.

* park.cpp안에 있다.
* 제작자 - 이호민
* park_tree_n함수 제작자 - 

# reset_park_position함수
* 이 함수는 공원이 형성될 장소의 공간을 BLOCK_AIR로 초기화해 건축물을 지우기 위한 함수입니다.
* x, y, z를 입력받아 127 * 50 * 127크기의 공간을 BLOCK_AIR로 가득 채웁니다.

* park.cpp안에 있다.
* 제작자 - 이호민

# 마인크래프트 쉐이더 사용법
* 위의 동영상 쵤영에는 "SEUS PTGI E6"쉐이더와 "umsoea r5 x1024", "Ultimate Immersion x1024 v0.2"리소스팩이 사용되었습니다. 사용한 쉐이더는  광원추적효과를 가지고있는 쉐이더로 아주 높은 사양의 그래픽을 요구합니다.
    - 적용법: 우선 쉐이더를 적용하기에 앞서 "OptiFine"모드가 필요합니다. 해당 마인크래프트 버전에 맞추어 "OptiFine"모드를 다운받고 "C:\Users\사용자이름\AppData\Roaming\CoalaMC\mods"폴더 안에 넣어줍니다.
    - 쉐이더 적용: 쉐이더를 적용하기 위해서 "SEUS PTGI E6"쉐이더를 다운받고 "C:\Users\사용자이름\AppData\Roaming\CoalaMC\shaderpacks"폴더 안에 넣어줍니다.
    - 리로스팩 적용: 리소스팩을 적용하기 위해서 "umsoea r5 x1024", "Ultimate Immersion x1024 v0.2"리소스팩을 다운받고"C:\Users\사용자이름\AppData\Roaming\CoalaMC\resourcepacks"폴더 안에 넣어줍니다.
    - 게임내에서 사용: 설정에 들어가서 리소스팩을 누르면 리소스팩을 활성화할 수 있습니다. 설정 -> 비디오 설정 -> 셰이더의 경로로 들어가서 셰이더를 활성화 해줍니다.

* <주의>: 광원추적효과를 내주는 해당 쉐이더는 매우 고사양의 그래픽사양을 요구합니다. 충분한 램을 할당하고 옵션을 적당히 조절하여 자신의 컴퓨터 사양에 맞게 사용하시길 바랍니다.
