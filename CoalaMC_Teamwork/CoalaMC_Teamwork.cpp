...
void LHM_building_1(int x, int y, int z) {		//이호민 빌딩 1 (25, 12, 15)
	...
	//0층부터 4층까지 한 블럭 0, 1, 2, 3, 4;
	
	//층수를 난수 만큼 생성하는 반복문
	for (int p = 0; p < creating_random_number(1, 3); p++) {
		tmpy = y + (p * 5);
		for (int q = 0; q < 5; q++) {
			tmpx = x;
			for (int i = 0; i < 25; i++) {
				...
				for (int j = 0; j < 15; j++) {
					...
					else if (arr[i][q][j] < 51) locateBlock(b[arr[i][q][j]], tmpx, tmpy, tmpz);
